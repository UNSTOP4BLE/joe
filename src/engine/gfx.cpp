...
#else    
    engine.window = SDL_CreateWindow("PSPFunkin", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
...
#else
    SDL_SetRenderDrawColor(engine.renderer, ((color >> 16) & 0xFF) / 255.0, ((color >> 8) & 0xFF) / 255.0, ((color) & 0xFF) / 255.0, 255);
...
#else
    Texture *psptex = NULL;
    psptex = IMG_LoadTexture(engine.renderer, path);
    assert(psptex);
    return psptex;
#endif
}
...
template<typename T> void drawTex(Texture* tex, GFX::RECT<int> *Img, GFX::RECT<T> *Disp, bool linear, float angle, int alpha) {
    ...
    if (Disp->x+Disp->w >= 0 && Disp->x <= SCREEN_WIDTH && Disp->y+Disp->h >= 0 && Disp->y <= SCREEN_HEIGHT)
    {
        ...
        assert(SDL_RenderCopy(engine.renderer, tex, &_img, &_disp) >= 0);
#endif
    }
}
